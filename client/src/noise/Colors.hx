...
class Colors {
    ...
    public static function rgb2hsl(rgb:RGB) : HSL {
        ...
        var h:Float =
            if (max == min) 0;
            else if (max == rgb.r) (60 * (rgb.g - rgb.b) / sub + 360) % 360;
            else if (max == rgb.g) 60 * (rgb.b - rgb.r) / sub + 120;
            else if (max == rgb.b) 60 * (rgb.r - rgb.g) / sub + 240;

        var l:Float = add / 2;

        var s:Float =
            if (max == min) 0;
            else if (l <= 1 / 2) sub / add;
            else sub / (2 - add);
