  ...
  public function fill( bitmap:BitmapData, _x:Float, _y:Float, _z:Float, _baseFactor:Float, _gain:Float, _offset:Float ,_clamp:Bool ,?_ ):Void {
    ...
    for ( py in 0...height )
    {
      ...
      for ( px in 0...width )
      {
        ...
        var num_octs = Math.floor((px / width) * octaves);
        if(num_octs != last_num_octs) {
            last_num_octs = num_octs;
            _x = baseX;
        }
        ...
        for ( i in 0...num_octs )
        {
            ...
            // Make the ridges.
            var sign = noise == 0 ? 0 : (noise > 0 ? 1 : -1);
			...
			weight = Math.abs(noise) * _gain;
			//if (weight > 1) weight = 1;
			//if (weight < 0) weight = 0;
			
			 // Add the signal to the output value.
            var addval = noise * fFreq;
            if(i < 6) {
                s += addval;
            } else {
                s += (addval * sign);
