...
class OptimizedPerlin {
    ...
    public function new( ?seed, ?octaves, ?falloff : Float ) {
    ...
    public function fill( bitmap:BitmapData, ?_x:Float=0, ?_y:Float=0, ?_z:Float=0, ?_ ):Void {
        ...
        for ( py in 0...height ) {
            ...
            for ( px in 0...width ) {
                ...
                for ( i in 0...octaves ) {
                    //s += inner_val(aOctFreq[i], aOctPers[i], _x, _y, _z, p);
                    s2+= inner_val(aOctFreq[i], aOctPers[i], _x, _y, _z + 30, p);
                    //s3+= inner_val(aOctFreq[i], aOctPers[i], _x, _y + 30, _z, p);
                }
                ...
                //bitmap.setPixel32( px, py, 0xff000000 | color << 8 | (color >> 1));
                //bitmap.setPixel32(px, py, /*0x00000000*/ color << 24 | (color3 >> 1) << 16 | color << 8 | color2 >> 3);
                //bitmap.setPixel32(px, py, /*0x00000000*/ color << 24 | (color3 >> 1) << 16 | color << 8);
    ...
    static inline function inner_val(fFreq:Float, fPers:Float, x:Float, y:Float, z:Float, p : Array<Int>) {
        x = x * fFreq;
        y = y * fFreq;
        z = z * fFreq;
